...
you can copy that URL and modify the "host" to be `localhost` or the domain you are using for development (e.g. `local.dockertoolbox.drosdek.com`), in the case above, it would be, e.g.:
...
As this is a common case, the domain `local.dockertoolbox.drosdek.com` points to that (private) IP, just to help with development (actually `dockertoolbox.drosdek.com` and all its subdomains point to that IP). That way, you can start the stack in Docker Toolbox, and use that domain for development. You will be able to open that URL in Chrome and it will communicate with your local Docker Toolbox directly as if it was a cloud server, including CORS (Cross Origin Resource Sharing).

If you used the default CORS enabled domains while generating the project, `local.dockertoolbox.drosdek.com` was configured to be allowed. If you didn't, you will need to add it to the list in the variable `BACKEND_CORS_ORIGINS` in the `.env` file.

To configure it in your stack, follow the section **Change the development "domain"** below, using the domain `local.dockertoolbox.drosdek.com`.

After performing those steps you should be able to open: http://local.dockertoolbox.drosdek.com and it will be server by your stack in your Docker Toolbox virtual machine.
...
In that case, you have two options: you could use the instructions to modify your system `hosts` file with the instructions below in **Development with a custom IP** or you can just use `localhost.drosdek.com`, it is set up to point to `localhost` (to the IP `127.0.0.1`) and all its subdomains too. And as it is an actual domain, the browsers will store the cookies you set during development, etc.

If you used the default CORS enabled domains while generating the project, `localhost.drosdek.com` was configured to be allowed. If you didn't, you will need to add it to the list in the variable `BACKEND_CORS_ORIGINS` in the `.env` file.

To configure it in your stack, follow the section **Change the development "domain"** below, using the domain `localhost.drosdek.com`.

After performing those steps you should be able to open: http://localhost.drosdek.com and it will be server by your stack in `localhost`.
...
If you need to use your local stack with a different domain than `localhost`, you need to make sure the domain you use points to the IP where your stack is set up. See the different ways to achieve that in the sections above (i.e. using Docker Toolbox with `local.dockertoolbox.drosdek.com`, using `localhost.drosdek.com` or using `dev.base-project.com`).
...
```
VUE_APP_DOMAIN_DEV=localhost.drosdek.com
```
...
You can use [`docker-auto-labels`](https://github.com/drosdek/docker-auto-labels) to automatically read the placement constraint labels in your Docker stack (Docker Compose file) and assign them to a random Docker node in your Swarm mode cluster if those labels don't exist yet.
...
Frontend: http://local.dockertoolbox.drosdek.com

Backend: http://local.dockertoolbox.drosdek.com/api/

Automatic Interactive Docs (Swagger UI): https://local.dockertoolbox.drosdek.com/docs

Automatic Alternative Docs (ReDoc): https://local.dockertoolbox.drosdek.com/redoc

PGAdmin: http://local.dockertoolbox.drosdek.com:5050

Flower: http://local.dockertoolbox.drosdek.com:5555

Traefik UI: http://local.dockertoolbox.drosdek.com:8090
...
Frontend: http://localhost.drosdek.com

Backend: http://localhost.drosdek.com/api/

Automatic Interactive Docs (Swagger UI): https://localhost.drosdek.com/docs

Automatic Alternative Docs (ReDoc): https://localhost.drosdek.com/redoc

PGAdmin: http://localhost.drosdek.com:5050

Flower: http://localhost.drosdek.com:5555

Traefik UI: http://localhost.drosdek.com:8090
